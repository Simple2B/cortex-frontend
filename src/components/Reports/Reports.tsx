...
export default function Reports(): ReactElement {
  ...
  const getFileWithNewClients = async () => {
    try {
      ...
      console.log(
        "GET: error message (file new_clients)=>  ",
        new Error(error.message)
      );
      throw new Error(error.message);
    }
  ...
  const filterDateForReport = () => {
    ...
    const dateStart = new Date(
      data.startDate.toString().replace(/GMT.*$/, "GMT+0000")
    ).toISOString();
    ...
    const dateEnd = new Date(
      data.endDate.toString().replace(/GMT.*$/, "GMT+0000")
    ).toISOString();
    ...
    <>
      ...
      <div className={isOpenModal ? "modalOpen" : "modal"}>
        <div className="modal-content">
          <div className="close" onClick={() => setIsOpenModel(!isOpenModal)}>
            &times;
          </div>

          {type && type.value.toLowerCase() === "visits" ? (
          ...
          ) : type && type.value.toLowerCase() === "new_clients" ? (
            <div className="modalText">
          ...
          {((file &&
            ...
            <div className="btnsModal">
              <div
                ...
                onClick={() => setIsOpenModel(!isOpenModal)}
              >
                <CSVLink
                  className="csvLink"
                  data={file}
